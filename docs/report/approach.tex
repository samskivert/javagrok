...
(Java Annotation Index File) format for that purpose. Twp analyses provide 
annotations embedded in byte-code which can be easily extracted into the JAIF 
format, and one provides textual results.

Figure~\ref{fig:from_source_to_jaif} 
illustrates the stages from the source to JAIF in JavaGrok's toolchain, explained 
...
During the compilation process the JDK~7 compiler~\cite{jdk7} reads in Uno's results 
to add annotations at the correct places. 
At the end of this compilation the class files contain retention and uniqueness
annotations.
Analog to that the exception analysis is integrated into the JDK~7 compiler too. 
The output are also class files with exception annotations.

JastAdd on the other hand processes the source code and adds nullability 
...
At this point JavaGrok has the annotations in class files for all three analyses.
