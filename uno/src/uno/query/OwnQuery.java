...
public class OwnQuery extends Query{
	...
	public OwnQuery(SootMethod method,int index){
		this.method = method;
		this.index = index;
		ID = 5;
	}
	...
	protected List[] getQueries() {
		...
		queries[2].add(QueryFactory.getNotLeakParQuery(method,index));
		...
		for(Iterator i=uno.Hierarchy.getSubMethodOf(method).iterator();i.hasNext();){
			...
			queries[0].add(QueryFactory.getOwnQuery(subm,index));
		}
		...
		if(superORthis!=null){
			for(int i=superORthis.getParameterCount()-1;i>=0;i--){
				if(share(p.getPar(index),p.get_superORthis_par_TO_obj(superORthis,i))){
					queries[4].add(QueryFactory.getOwnQuery(superORthis,i));
					needStore = false;
				}
			}
		}
		if(needStore) queries[3].add(QueryFactory.getStoreQuery(method,index));
	...
	protected void setName() {
		...
		name = "Own ( " + simplify(method) + " , " + index + " ) " + (isCalled?" *never called* ":" *called* ") ;
	}
}
